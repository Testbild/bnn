...
class LabelUI():
    def __init__(self, label_db_filename, img_dir, width, height, sort=True):
        ...
        print("Q   quit")
        print("w,a,s,d   to navigate")
        print("##############################")
        print("YOU NEED TO MOVE TO THE NEXT/PREVIOUS IMAGE TO SAVE THE LABELS")
        print("##############################")

        self.canvas = tk.Canvas(root, cursor='tcross')

        vscrollbar = tk.Scrollbar(root, command=self.canvas.yview)
        hscrollbar = tk.Scrollbar(root, orient='horizontal', command=self.canvas.xview)
    ...
    def add_bee_event(self, e):
        if not self.bees_on:
            ...
            return
        x_scrolled = self.canvas.canvasx(e.x)
        y_scrolled = self.canvas.canvasy(e.y)

        self.add_bee_at(x_scrolled, y_scrolled)
    ...
    def remove_closest_bee_event(self, e):
        ...
        for x, y in self.x_y_to_boxes.keys():
            sqr_distance = (self.canvas.canvasx(e.x) - x) ** 2 + (self.canvas.canvasx(e.y) - y) ** 2
    ...
    def display_new_image(self):
        ...
        # maxwidth = 1600
        # maxheight = 1600
        # ratio = min(maxwidth/4896, maxheight/3672)
        # print(ratio)
        # resize_width = ratio* 4896
        # resize_height = ratio* 3672
        # img = img.resize((int(resize_width), int(resize_height)), Image.BOX)
        # print("Resize End")
